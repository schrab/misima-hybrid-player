...
# 14 vertical slots on bg (from track scan) — each fader gets ONE unique slot
SLOTS = [
...
for i, (fid, param, png, rng, val) in enumerate(META):
    ...
    s = SLOTS[i]  # UNIQUE slot per fader, 1:1 L→R
    top = s["y0"]
    travel = max(80, s["y1"] - s["y0"] - h)
    tops.append(top)
    faders.append({
        "id": fid,
        "param": param,
    ...
    print(f"{fid:8} x={s['x']:4} maxY={top:4} travel={travel:3} size={w}x{h}")

assert len(set(tops)) == 14, f"max Y must all differ, got {tops}"
